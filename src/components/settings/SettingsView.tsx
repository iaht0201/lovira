...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  useRegisterScreenActions({
    ...
    actions: [
      ...
      {
        id: 'toggleVSL',
        label: 'Bật/Tắt ngôn ngữ ký hiệu VSL',
        aliases: ['ngôn ngữ ký hiệu', 'ký hiệu vsl', 'bật ký hiệu', 'tắt ký hiệu'],
        description: 'Bật hoặc tắt hình nhân mô phỏng ngôn ngữ ký hiệu VSL',
        handler: () => onUpdateSettings({ vslAccessibilityEnabled: !settings.vslAccessibilityEnabled }),
      },
      {
        id: 'testVoice',
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {/* 4. Vietnamese Sign Language (VSL) Accessibility Settings */}
      <section className="bg-surface border border-slate-200 dark:border-slate-800 p-6 rounded-2xl space-y-6">
        <h3 className="text-base font-bold text-text-primary flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
          <HandMetal className="w-5 h-5 text-sky-500 shrink-0" /> Ngôn ngữ Ký hiệu Việt Nam (VSL)
        </h3>
        <p className="text-xs text-text-secondary leading-relaxed">
          Tự động hiển thị nhân vật mô phỏng ngôn ngữ ký hiệu 5 ngón ở góc phải màn hình mỗi khi Voice Action hoặc hệ thống phát câu thoại.
        </p>
        ...
        <div className="flex items-center justify-between p-3.5 rounded-xl bg-surface-subtle border border-slate-100 dark:border-slate-800">
          <div className="space-y-0.5 pr-4">
            <span className="text-sm font-bold text-text-primary flex items-center gap-2">
              Bật trợ năng Ngôn ngữ ký hiệu VSL
            </span>
            <p className="text-xs text-text-secondary">
              Hiển thị khung hình nhân ký hiệu nổi cố định ở góc dưới bên phải, đồng bộ trực tiếp với câu nói của Voice Action.
            </p>
          </div>

          <button
            type="button"
            id="vsl-accessibility-toggle-btn"
            onClick={() => onUpdateSettings({ vslAccessibilityEnabled: !settings.vslAccessibilityEnabled })}
            className={`w-11 h-6 rounded-full p-0.5 transition-colors shrink-0 ${
              settings.vslAccessibilityEnabled ? 'bg-primary' : 'bg-slate-300 dark:bg-slate-700'
            }`}
            aria-label="Bật hoặc tắt Ngôn ngữ ký hiệu VSL"
          >
            <div
              className={`w-5 h-5 rounded-full bg-white transition-transform ${
