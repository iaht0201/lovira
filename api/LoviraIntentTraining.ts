/**
 * Lovira Production Few-Shot Training Corpus & Semantic Knowledge Base
 * 
 * Provides curated Vietnamese training datasets and few-shot pairs for:
 * 1. Exact & Colloquial Route Navigation
 * 2. Lovira Life Scenarios (Healthcare, Administrative, Shopping, Reading)
 * 3. Multimodal Conversational Memory & Contextual Inquiries ("Giờ làm gì?", "Tôi còn thiếu gì?")
 * 4. Accessibility & Screen Control Actions
 */
...
export const LOVIRA_FEW_SHOT_TRAINING_CORPUS: TrainingExample[] = [
  // =========================================================================
  // 1. ROUTE NAVIGATION (TRANG CHỦ & CÁC ROUTER CHỨC NĂNG)
  // =========================================================================
  {
  ...
  // =========================================================================
  // 2. LOVIRA LIFE SCENARIOS (KHÁM BỆNH, THỦ TỤC, MUA SẮM, HỌC TẬP)
  // =========================================================================
  {
  ...
  // =========================================================================
  // 3. CONVERSATIONAL MEMORY & FOLLOW-UP INQUIRIES ("GIỜ LÀM GÌ?", "TÔI CÒN THIẾU GÌ?")
  // =========================================================================
  {
  ...
  {
    userUtterance: 'Làm lại thao tác vừa nãy',
    context: { currentScreen: 'vision', currentRoute: '/vision', hasImage: true },
    expectedAction: 'RETRY_LAST_ACTION',
    expectedPlan: [{ action: 'vision.analyze', reason: 'Thực hiện lại bước phân tích ảnh' }],
    feedback: 'Lovira đang thực hiện lại thao tác cho bạn.',
    category: 'memory_query',
  },

  // =========================================================================
  // 4. ACCESSIBILITY & DISPLAY CONTROLS
  // =========================================================================
  {
...
/**
 * Serializes the training corpus into concise Few-Shot JSON blocks for inclusion in LLM System Instructions.
 */
export function formatFewShotPromptExamples(): string {
  return LOVIRA_FEW_SHOT_TRAINING_CORPUS.map((ex, i) => `[VÍ DỤ HUẤN LUYỆN ${i + 1}]
- Người dùng nói: "${ex.userUtterance}"
- Ngữ cảnh: ${JSON.stringify(ex.context)}
- Phản hồi chuẩn (JSON):
${JSON.stringify(
  {
    action: ex.expectedAction,
    confidence: 1.0,
    plan: ex.expectedPlan,
